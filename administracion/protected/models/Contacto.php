...
class Contacto extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('nombre, apPaterno, correo, estatus_did, fechaCreacion_f', 'required'),
			array('estatus_did', 'numerical', 'integerOnly'=>true),
			array('nombre, apPaterno, apMaterno, correo', 'length', 'max'=>50),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, nombre, apPaterno, apMaterno, correo, estatus_did, fechaCreacion_f', 'safe', 'on'=>'search'),
		);
	}
